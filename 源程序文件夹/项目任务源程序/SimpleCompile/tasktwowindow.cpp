    ...
    QString filePath = QFileDialog::getOpenFileName(this, "打开编码文件", QDir::currentPath(), "Text Files (*.txt *.lex *.minic)");
       ...
       while (!in.atEnd()) {
           ...
           if (regex.indexIn(line) != -1) {
               QString code = regex.cap(1).trimmed();
               QString value = regex.cap(2).trimmed();

               sourceDisplay->insertRow(row);
               sourceDisplay->setItem(row, 0, new QTableWidgetItem(code));
               sourceDisplay->setItem(row, 1, new QTableWidgetItem(value));
               row++;

               //加入token
               tokens.push_back(Token{code.toInt(), value.toStdString()});
           }
       }
       file.close();
